...
namespace DS.Windows
{
    ...
    public class DSGraphView : GraphView
    {
        ...
        private IManipulator CreateNodeContextualMenu(string actionTitle, DSDialogueType dialogueType,string node_name = "DialogName")
        {
            ContextualMenuManipulator contextualMenuManipulator = new ContextualMenuManipulator(
                menuEvent => menuEvent.menu.AppendAction(actionTitle, actionEvent => AddElement(CreateNode(node_name, dialogueType, GetLocalMousePosition(actionEvent.eventInfo.localMousePosition))))
            );
        ...
        public DSNode CreateNode(string nodeName, DSDialogueType dialogueType, Vector2 position, bool shouldDraw = true)
        {
            ...
            switch (node)
            {
                case DSConnectorNode:
                    if (selection.Count == 1 && selection[0] is Edge)
                    {
                        Debug.Log("Connecting Connector node");
                        Edge edge = (Edge)selection[0];
                        Node inputNode = edge.input.node;
                        Node outputNode = edge.output.node;
                        Port originalInputPort = edge.input;
                        Port originalOutputPort = edge.output;
                        edge.input.Disconnect(edge);
                        edge.output.Disconnect(edge);
                        Edge newEdgeIn = originalInputPort.ConnectTo(node.outputContainer.Q<Port>());
                        AddElement(newEdgeIn);

                        Edge newEdgeout = originalOutputPort.ConnectTo(node.inputContainer.Q<Port>());
                        AddElement(newEdgeout);


                        DSChoiceSaveData choiceData = (DSChoiceSaveData)outputNode.outputContainer.Q<Port>().userData;
                        choiceData.NodeID = node.ID;

                        DSNode lastNode = (DSNode)inputNode;
                        DSChoiceSaveData currentNodeChoiceData = (DSChoiceSaveData)node.outputContainer.Q<Port>().userData;
                        currentNodeChoiceData.NodeID = lastNode.ID;
        ...
        public void AddUngroupedNode(DSNode node)
        {
            ...
            if (node is DSConnectorNode) return;
            if (node is DSReturnToStartNode) return;
            if (node is DSCloseDialogNode) return;
